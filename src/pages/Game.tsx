...
import { ABANDON_ENERGY_COST, abandonChallenge, answerQuestion, createGame, DIFFICULTIES, isAdjacent, move } from '../game/engine';
import type { Difficulty, GameState } from '../game/engine';
import { sound, unlockAudio } from '../game/audio';
export default function Game({ onWin, onHelp }: { onWin: (energy: number, monsters: number, level: number) => void; onHelp: () => void }) {
  ...
  function restart(newDifficulty = state.difficulty, nextLevel = state.level, sameMap = true) { clearTimeout(timerRef.current); endRef.current = false; setCelebrate(false); setFeedback(null); setFlash(''); setSettings(false); setResetConfirm(false); setPaused(false); commit(createGame(newDifficulty, sameMap ? state.seed : Math.floor(Math.random() * 1e8), nextLevel)); }
  function choose(answer: number) {
    if (feedback !== null || stateRef.current.status !== 'question') return;
    unlockAudio(); const current = stateRef.current;
    const next = answerQuestion(current, answer);
    if (next.status === 'lost') { commit(next); return; }
    if (next.status === 'question') { commit(next); sound('drain'); return; }
    sound('correct'); setFeedback(answer);
    timerRef.current = setTimeout(() => { setFeedback(null); commit(next); }, 650);
  }
  function giveUp() {
  ...
  {state.status==='question'&&state.question&&<Modal title="小怪兽的数学挑战" className="question-modal"><div className="question-monster"><Monster/><span>?</span></div><span className="challenge-tag">小怪兽的脑力挑战</span><h2>{feedback===null?'先观察，再算一算！':'答对啦，你真聪明！'}</h2><p>不用急，我们用图形把数字变出来。</p><div className="math-visual" aria-label={`${state.question.a}个和${state.question.b}个图形`}><MathBeans question={state.question}/></div><div className="equation">{state.question.a} <span>{state.question.operator}</span> {state.question.b} <span>=</span> <b>?</b></div><div className="math-tip"><Sparkles size={15}/><span><b>{({ 'count-all':'数一数', 'make-ten':'凑十法', 'split-number':'拆开来算', 'double':'想双倍' } as const)[state.question.strategy]}</b> · {({ 'count-all':'把两组图形合在一起数', 'make-ten':`先把 ${state.question.a} 凑成 10`, 'split-number':'把大数字拆成容易计算的部分', 'double':'两个一样多的数，可以想成双倍' } as const)[state.question.strategy]}</span></div><div className="answer-grid">{state.question.choices.map(n=><button key={n} className={`${feedback===n?'correct-answer':''} ${state.question?.rejected.includes(n)?'rejected-answer':''}`} disabled={feedback!==null} onClick={()=>choose(n)}>{n}{feedback===n&&<Check size={21}/>}</button>)}</div>{state.question.rejected.length > 0 && <p className="retry-hint">再试一次：刚才的答案已经标记啦，剩下的选项更有希望。</p>}<button className="button abandon-button" onClick={giveUp} disabled={feedback !== null}><ArrowLeft size={17}/>放弃挑战<span>−{ABANDON_ENERGY_COST} 能量 · 留在原地</span></button><div className="question-foot"><Sparkles size={14}/> 让小脑袋发光的时刻到了</div></Modal>}
...
function MathBeans({ question }: { question: GameState['question'] }) {
  if (!question) return null;
  const row = (count: number, kind: 'first' | 'second') => <div className={`math-line group-${kind}`} aria-label={`${count} 个豆子`}><div className="bean-rows">{Array.from({ length: Math.ceil(count / 5) }, (_, group) => <div className="bean-row" key={group}>{Array.from({ length: Math.min(5, count - group * 5) }, (_, index) => { const absolute = group * 5 + index; const removed = question.operator === '−' && kind === 'first' && absolute < question.b; const extra = question.operator === '−' && kind === 'first' && absolute >= question.b; return <i key={absolute} className={`${removed ? 'bean-removed' : ''} ${extra ? 'bean-extra' : ''}`} style={{ '--i': absolute } as CSSProperties}>●</i>; })}</div>)}</div></div>;
  return <div className="math-lines"><div className="math-line-label">{question.a}</div>{row(question.a, 'first')}<span className="visual-operator">{question.operator}</span><div className="math-line-label">{question.b}</div>{row(question.b, 'second')}</div>;
}
