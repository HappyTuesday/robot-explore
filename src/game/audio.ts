...
export function sound(type: 'move' | 'boost' | 'drain' | 'correct' | 'win' | 'lose' | 'click') {
  ...
  try {
    ...
    if (type === 'lose' || type === 'win') {
      ...
      for (let i = 0; i < data.length; i++) { const time = i / ctx.sampleRate; data[i] = (Math.random() * 2 - 1) * (type === 'win' ? Math.pow(Math.max(0, Math.sin(time * 47)), 12) * .32 * (1 - time / length) : Math.exp(-time * 9) * .6); }
      const source = ctx.createBufferSource(); source.buffer = buffer; source.connect(ctx.destination); source.start(t + (type === 'win' ? .2 : 0));
      if (type === 'win' && 'speechSynthesis' in window) { const voice = new SpeechSynthesisUtterance('耶！太棒啦！你成功了！'); voice.lang = 'zh-CN'; voice.rate = 1.15; voice.pitch = 1.45; voice.volume = .65; window.speechSynthesis.speak(voice); }
    }
